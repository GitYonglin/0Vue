      ...
      <div>
        ...
        <div>
          <el-tabs v-model="activeTabs" type="card" @tab-click="tabsClick">
            ...
            <!-- <el-tab-pane label="主料" name="1" ></el-tab-pane> -->
            <!-- <el-tab-pane label="辅料" name="2"></el-tab-pane>
            <el-tab-pane label="酱料" name="3"></el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    getData(name = '') {
      ...
      let url = `${this.$store.state.apiUrl}/food?foodCategory_like=${Number(this.activeTabs) + 1}&name_like=${name}`
      this.$axios({
        method: 'get',
        url: url
      }).then(response => {
    ...
    ok() {
      ...
      try {
        ...
        this.$axios.all([
          this.$axios.get(`${this.$store.state.apiUrl}/food?id=${urls[0]}`),
          this.$axios.get(`${this.$store.state.apiUrl}/food?id=${urls[1]}`),
          this.$axios.get(`${this.$store.state.apiUrl}/food?id=${urls[2]}`),
          this.$axios.get(`${this.$store.state.apiUrl}/food?id=${urls[3]}`)
        ]).then(response => {
    ...
    sumPrice(id, index, e) {
      ...
      this.data.foodsPrice = price
      if (id && !this.data.cookbooksPrice) {
        this.data.cookbooksPrice = 0
      }
      if (id) {
        this.data.unitPrice = (price + this.data.cookbooksPrice).toFixed(2)
      }
