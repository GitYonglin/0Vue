      ...
      <el-tabs v-model="activeName">
        <el-tab-pane label="基本信息" name="a">
          ...
          <el-form-item label="价格" prop="unitPrice">
            <el-input type="unitPrice" v-model.number="formData.unitPrice" :disabled="true">
        ...
        <el-tab-pane label="图文教程" name="d">
          <el-button type="primary" @click="addpictureText(formData.pictureTextCoures)" size="small">添加</el-button>
          <picture-text :pictureText.sync="formData.pictureTextCoures" ></picture-text>
        </el-tab-pane>
        <el-tab-pane label="生产资料" name="e">生产资料</el-tab-pane>
...
// import qs from 'qs';
// import SelectFood from './select'
import SelectOk from './selectOK'
...
export default {
  ...
  components: {
    SelectFood, SelectOk, GoodsCategorySelect, PictureText
  },
  // 界面渲染前处理数据
  beforeMount () {
    this.formData = this.$unity.copyObj(this.$store.state.base.cookbookBase)
    // 检测新增或编辑
    if (this.$route.params.id) {
      this.subTitle = '修改保存'
      this.$axios({
        method: 'get',
        url: `${this.$store.state.apiUrl}/cookbook?id=${this.$route.params.id}`
      }).then(response => {
        this.formDataBase = response.data[0]
        this.resetForm()
      })
    } else {
      this.formDataBase = this.$unity.copyObj(this.$store.state.base.cookbookBase)
    }
  },
  methods: {
    // 测试fromdata转换数据
    ...
    // 数据提交到后台
    submitForm() {
      // this.foodeSelectDodage()
      this.formData.goodsCategory = this.syncgoodsCategory

      let m = 'patch'
      let url = `${this.$store.state.apiUrl}/cookbook/${this.formData.id}`
      let msg = '菜谱修改成功'
      if (!this.formData.id) {
        m = 'post'
        url = `${this.$store.state.apiUrl}/cookbook`
        msg = '菜谱添加成功'
        // 获取创建日期
      ...
      this.$refs.formData.validate((result) => {
        if (result) {
          ...
          this.$axios({
            ...
            this.$router.push('/goods/cookbook')
            this.$notify({
              title: '成功',
              message: msg,
              type: 'success'
